...
//    }
    ...
    if (plusIndex != -1 || minusIndex != -1) {
        // Find the index of the first '+' or '-' character that is not enclosed in parentheses
        int index = -1;
        char op;
        if (plusIndex != -1 && !enclosedChar(text, plusIndex)) {
            index = plusIndex;
            op = '+';
        } else if (minusIndex != -1 && !enclosedChar(text, minusIndex)) {
            index = minusIndex;
            op = '-';
        }
        if (index != -1) {
            // Parse the term before the '+' or '-' character as term2
            char *term2 = sliceString(text, 0, index);
            parseTerm2(term2);
            // Append the '+' or '-' character to the parse string
            char opStr[2] = {op, '\0'};
            strcat(parseString, opStr);
            // Parse the term after the '+' or '-' character as term1
            char *internalTerm1 = sliceString(text, index + 1, strlen(text));
            parseTerm1(internalTerm1);
        } else {
            // If there are no '+' or '-' characters that are not enclosed in parentheses, parse the whole string as term2
            char *term2 = text;
            parseTerm2(term2);
        }
    } else {
        // If there are no '+' or '-' characters, parse the whole string as term2
        char *term2 = text;
        parseTerm2(term2);
    }
